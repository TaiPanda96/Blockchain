...
const register = async (req, res) => {
    ...
    try {
        User.create({ ...userPayload }).then(async (newUser) => {
            ...
            eventEmitter.emit('risk-review', newUser)
...
const login = async (req, res) => {
    ...
    try {
        ...
        bcrypt.compare(password, user.password).then(async (verify) => {
            if (verify === false) { return res.status(401).send({ message: "Login Unsuccessful" }) };
